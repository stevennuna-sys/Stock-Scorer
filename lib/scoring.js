...
// ─── FMP INTERPRETER ──────────────────────────────────────────────────────────
// Converts Financial Modeling Prep (FMP) response into the same
// factorId → { index, confidence, raw } shape that the UI autofill expects.
// Only implements fields that FMP stable endpoints can reliably supply.

/**

- EPS Surprise from FMP earningsSurprises array.
- FMP field: surprisePercentage (already a percentage, e.g. 8.3 means 8.3%)
- Bucket spec (from product requirements):
- = 15%   → 4
- 7-14.99% → 3
- -2.99 to 6.99% → 2
- -9.99 to -3%   → 1
- <= -10%  → 0
  */
  ...
  try {
  const surprises = fmpData?.earningsSurprises;
  if (!Array.isArray(surprises) || !surprises.length) {
  return { index: null, confidence: “low”, raw: null };
  }
  // Most recent quarter is first in FMP response
  const recent = surprises[0];
  const pct = recent?.surprisePercentage ?? recent?.surprise_percentage ?? null;
  const actualEps   = recent?.actualEarningResult ?? recent?.actual ?? null;
  const estimateEps = recent?.estimatedEarning   ?? recent?.estimate ?? null;
...
return {
  ...
  confidence: 0.7,
  raw: {
    surprisePct: +p.toFixed(2) + "%",
    actual: actualEps,
    estimate: estimateEps,
    period: recent?.date || recent?.period || null,
  },
};
```
...
- Relative Valuation from FMP quote[0].pe
- Re-uses the same sector lookup table and bucket logic as scoreValuation().
- Sector is pulled from profile[0].sector.
  */
  ...
  const pe = quote?.pe ?? quote?.priceEarningsRatio ?? null;
  if (pe === null || pe === undefined) {
  return { index: null, confidence: “low”, raw: { pe: null } };
  }
  
  const peNum = Number(pe);
  if (isNaN(peNum) || peNum <= 0) {
  return { index: null, confidence: “low”, raw: { pe: peNum } };
  }
  
  // Sector from FMP profile matches our SECTOR_FWD_PE keys (same Morningstar taxonomy)
...
- Company name helper — used to update stock name if it’s still the placeholder.
- Returns { index: null } because it’s not a scored factor; the UI reads .raw.companyName.
  */
  function fmpCompanyName(fmpData) {
  try {
  const profile = fmpData?.profile?.[0];
  const name = profile?.companyName || profile?.name || null;
  if (!name) return { index: null, confidence: 1, raw: null };
  return { index: null, confidence: 1, raw: { companyName: name } };
  } catch {
  return { index: null, confidence: 1, raw: null };
  }
  }

/**

- Master FMP interpreter.
- Returns factorId → { index, confidence, raw } for all fields we can infer.
- Fields that cannot be inferred from FMP stable endpoints are omitted.
  */
  ...
  // These require endpoints not in the stable FMP plan used here:
  // revisions, revision_velocity, revenue_momentum, eps_inflection,
  // catalyst_proximity, chart_trend, accumulation, balance_sheet, macro_sensitivity
  };
  }
